...
export function GoalSettingTab({ userState, originalUserRole, onShowAI, simulationData, onSimulationDataUsed }: GoalSettingTabProps) {
  ...
  // Auto-populate quarterly Personal FYC from monthly goal FYC for leaders
  // This ensures quarterly FYP values are calculated correctly when goals are saved
  // Only auto-populates when monthly goal FYC changes and quarterly values are empty
  useEffect(() => {
    if (!isLeader || !dataLoaded) return; // Only for leaders and after data is loaded
    
    const monthlyFYC = parseCommaNumber(monthlyGoalFYC) || 0;
    
    // Only auto-populate if monthly goal FYC is set and quarterly values are empty or zero
    // This allows manual editing while still providing auto-population for new entries
    if (monthlyFYC > 0) {
      // Check if quarterly values are empty or zero - only then auto-populate
      const q1Current = parseCommaNumber(q1PersonalFYC) || 0;
      const q2Current = parseCommaNumber(q2PersonalFYC) || 0;
      const q3Current = parseCommaNumber(q3PersonalFYC) || 0;
      const q4Current = parseCommaNumber(q4PersonalFYC) || 0;
      
      // Only auto-populate if all quarterly values are empty/zero
      // This prevents overwriting manually entered values
      if (q1Current === 0 && q2Current === 0 && q3Current === 0 && q4Current === 0) {
        const quarterlyFYC = monthlyFYC * 3; // Quarterly = 3 months
        const quarterlyFYCFormatted = formatNumberWithCommas(Math.round(quarterlyFYC).toString());
  ...
  }, [monthlyGoalFYC, isLeader, dataLoaded]);
  ...
  const handleSubmitGoals = async () => {
    ...
    try {
      ...
      // Safety check: If quarterly FYP is 0 but monthly goal FYP is set and quarterly Personal FYC is 0,
      // it means quarterly Personal FYC wasn't auto-populated. Recalculate from monthly goal.
      // This handles edge cases where auto-population didn't trigger (e.g., for existing goals)
      if (isLeader && monthlyTargetFYP > 0) {
        const quarterlyPersonalFYPFromMonthly = (monthlyTargetFYP * 3); // Quarterly = 3 months
        const quarterlyPersonalFYCFromMonthly = monthlyTargetFYC * 3; // Quarterly = 3 months
        
        quarters.forEach((q, index) => {
          // Get the corresponding quarterly Personal FYC value
          const qPersonalFYC = index === 0 ? (parseCommaNumber(q1PersonalFYC) || 0) :
                               index === 1 ? (parseCommaNumber(q2PersonalFYC) || 0) :
                               index === 2 ? (parseCommaNumber(q3PersonalFYC) || 0) :
                               (parseCommaNumber(q4PersonalFYC) || 0);
          
          // Get the corresponding quarterly Team FYC value
          const qTeamFYC = index === 0 ? (parseCommaNumber(q1TeamFYC) || 0) :
                          index === 1 ? (parseCommaNumber(q2TeamFYC) || 0) :
                          index === 2 ? (parseCommaNumber(q3TeamFYC) || 0) :
                          (parseCommaNumber(q4TeamFYC) || 0);
          
          // If Personal FYC is 0 but monthly goal is set, use monthly goal to calculate
          // Only do this if the total FYC (Personal + Team) is 0 or very small
          if (qPersonalFYC === 0 && monthlyTargetFYC > 0 && q.fyc <= qTeamFYC) {
            // Recalculate quarterly values from monthly goal
            const newQPersonalFYC = quarterlyPersonalFYCFromMonthly;
            const newQPersonalFYP = quarterlyPersonalFYPFromMonthly;
            const newQTotalFYC = newQPersonalFYC + qTeamFYC;
            const newQTotalFYP = newQPersonalFYP + (qTeamFYC / rate);
            
            // Update the quarter's FYC and FYP
            q.fyc = newQTotalFYC;
            q.fyp = newQTotalFYP;
          }
        });
      ...
      // Check if this is an update to existing goals
      const existingGoal = await getUserGoal(userState.uid, userState.agency);
      const isUpdating = !!existingGoal;
      
      // If updating, show confirmation
      if (isUpdating) {
        const confirmed = window.confirm(
          'You have previously submitted goals for this agency. This will overwrite your previous submission.\n\n' +
          'Do you want to continue?'
        );
        
        if (!confirmed) {
          setIsSubmitting(false);
          setSubmitMessage({ type: 'info', text: 'Submission cancelled.' });
          return;
        }
      }
      ...
      if (saveResult.success) {
        const message = saveResult.isUpdate 
          ? 'Goals updated successfully! Your previous submission has been overwritten.'
